...
// with minor changes to the I/O pin definitions. It is suitable for controlling all of
// our I2C based robot modules. Since the SCL and SDA lines are open drain type, we use
// the tristate control register to control the output, keeping the output register low.
// The port pins still need to be read though, so they're defined as SCL_IN and SDA_IN.
// This definition and the initialization is probably all you'll need to change for a different
// processor.
...
// The following 4 functions provide the primitive start, stop, read and write sequences. All I2C transactions
